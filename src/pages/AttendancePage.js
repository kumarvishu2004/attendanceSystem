...
function AttendancePage() {
  ...
  // actionMode is set explicitly by the Login / Logout buttons below.
  // null = no action chosen yet (selection screen is shown)
  const [actionMode, setActionMode] = useState(null); // 'login' | 'logout' | null
  ...
  // Can the user press Login? Only if they haven't logged in yet today.
  const canLogin = !isLoggedIn;
  // Can the user press Logout? Only if they're logged in but haven't logged out yet.
  const canLogout = isLoggedIn && !isLoggedOut;
  ...
  // Called when the user taps the Login or Logout button
  const startAction = (selected) => {
    setActionMode(selected);
    resetScanState();
  };

  // Go back to the Login/Logout selection screen
  ...
  const handleFaceDetected = useCallback(async (descriptor) => {
    if (!actionMode || processingRef.current || status === 'success') return;
    ...
    try {
      ...
      if (matched && matchUser) {
        ...
        if (consecutiveMatches.current >= CONFIRM_FRAMES) {
          // Confirmed match — process attendance for the chosen action
          consecutiveMatches.current = 0;
  ...
  }, [status, actionMode, fetchTodayAttendance]);
    ...
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          {!actionMode
            ? 'Choose Login when you arrive, or Logout when you leave the office'
            : actionMode === 'login'
              ? 'Look at the camera to mark your login'
              : 'Look at the camera to mark your logout'}
        </p>
      </div>
      ...
      {isLoggedOut ? (
        ...
        /* ─── Selection screen: two explicit buttons ─── */
        <div className="card text-center py-10 space-y-5">
          <div className="text-4xl">🕒</div>
          <div>
            <h3 className="text-lg font-bold text-white">Mark Your Attendance</h3>
            <p className="text-gray-500 text-sm mt-1">Pick an action to start face verification</p>
          </div>

          <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto">
            <button
              onClick={() => startAction('login')}
              disabled={!canLogin}
              className={`py-6 rounded-xl font-semibold text-white transition-all ${
                canLogin ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-gray-700/60 opacity-40 cursor-not-allowed'
              }`}
            >
              <div className="text-2xl mb-1">📥</div>
              <div>Login</div>
              <div className="text-xs font-normal opacity-80 mt-0.5">Check In</div>
            </button>
            ...
            <button
              ...
              disabled={!canLogout}
              className={`py-6 rounded-xl font-semibold text-white transition-all ${
                canLogout ? 'bg-red-600 hover:bg-red-500' : 'bg-gray-700/60 opacity-40 cursor-not-allowed'
              }`}
            >
              <div className="text-2xl mb-1">📤</div>
              <div>Logout</div>
              <div className="text-xs font-normal opacity-80 mt-0.5">Check Out</div>
            </button>
          </div>

          <p className="text-gray-600 text-xs">
            {!isLoggedIn
              ? 'Tap Login when you arrive at the office.'
              : 'You\'re checked in — tap Logout when you leave the office.'}
          </p>
        </div>
      ) : (
        <>
          {/* Back to selection + current action badge */}
          <div className="flex items-center justify-between">
            <button onClick={backToSelection} className="text-gray-500 hover:text-gray-300 text-sm transition-colors">
              ← Back
            </button>
            <span className={`text-xs font-semibold px-3 py-1 rounded-full ${
              ...
              {actionMode === 'login' ? '📥 Login Mode' : '📤 Logout Mode'}
            </span>
          </div>
          ...
          <div className={`card text-center py-6 transition-all duration-300 ${
            ...
            {status === 'success' && (
              <div className="match-success">
                <div className="text-5xl mb-3">{actionMode === 'login' ? '👋' : '🏠'}</div>
                <p className="text-emerald-400 font-bold text-lg">{actionMode === 'login' ? 'Login Recorded!' : 'Logout Recorded!'}</p>
                <p className="text-gray-300 text-sm mt-2">{message}</p>
                <button onClick={backToSelection} className="btn-secondary mt-4 text-sm">
                  Done
                </button>
              </div>
            )}
            ...
            {(status === 'error' || status === 'alreadyDone') && (
              <div>
                ...
                <div className="flex gap-3 justify-center mt-4">
                  ...
                  <button onClick={backToSelection} className="btn-secondary text-sm">← Back</button>
                </div>
              </div>
            )}
          </div>
          ...
          {user?.faceRegistered === false && status !== 'success' && (
            <div className="card border-amber-800/30 bg-amber-900/10">
              ...
              <button
                ...
                className={`w-full text-sm ${actionMode === 'login' ? 'btn-success' : 'btn-danger'}`}
                disabled={loading}
              >
                Manual {actionMode === 'login' ? 'Login' : 'Logout'}
              </button>
            </div>
          )}
        </>
